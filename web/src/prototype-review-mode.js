...
export class AgentNudgeReview extends HTMLElement {
  ...
  askThread(threadId) {
    ...
    thread.conversation.push({ role: "user", text: message, referenceIds });
    thread.draft = "";
    thread.feedbackText = "";
    thread.asking = true;
    this.render();
    const timer = setTimeout(() => {
      ...
      if (!current) return;
      current.conversation.push({ role: "agent", text: prototypeAgentReply(snapshot), referenceIds });
  ...
  render() {
    ...
    this.sendButton.disabled = this.sending || this.threads.length === 0;
    this.renderHistoryButtons();
    this.renderStack();
  ...
  renderActiveThread() {
    ...
    titleMeta.textContent = thread.asking ? "Agent is replying" : "Add marks, ask, or queue";
    ...
    for (const message of thread.conversation) {
      ...
      speaker.textContent = message.role === "agent" ? "Prototype agent" : "You";
    ...
    if (thread.asking) {
      ...
      working.setAttribute("aria-label", "Prototype agent is replying");
    ...
    textarea.placeholder = thread.conversation.length ? "Reply or write the final feedback…" : "Write feedback or ask the agent…";
    textarea.setAttribute("aria-label", `Feedback thread ${thread.number}`);
    textarea.addEventListener("input", () => {
      thread.draft = textarea.value;
      autoSizeTextarea(textarea);
      ask.disabled = thread.asking || !thread.draft.trim();
    ...
    const ask = iconButton("Ask prototype agent", iconMessage, "thread-icon ask-thread");
    ask.disabled = thread.asking || !thread.draft.trim();
